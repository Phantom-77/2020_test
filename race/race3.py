# This is the "Race 3.0.3" program, where 2 cars driven by two players
import pygame
import sys
import winsound
...
def start_screen():
    ...
    screen.fill(BLACK)
    draw_car(xct-300, yct+100, '1', TEAL)
    draw_car(xct+200, yct+100, '2', GREEN)
    display_text_center('RACE 3', 54, 0, -140)
    display_text_center('Press SPACE - for start race', 22, 0, -80)
    display_text_center('Press Q - for quit game', 22, 0, -60)
    display_text_center('Press P - to view statistics', 22, 0, -40)
    display_text('Car 1 :', 22, xct-300, yct-40, TEAL)
    display_text('A - forward', 22, xct-300, yct-20, TEAL)
    display_text('D - back', 22, xct-300, yct, TEAL)
    display_text('Car 2 :', 22, xct+200, yct-40, GREEN)
    display_text('Left - forward', 22, xct+200, yct-20, GREEN)
    display_text('Right - back', 22, xct+200, yct, GREEN)
    pygame.display.update()
...
def statistics():
    ...
    display_text('Total Races: ', 24, xct-75, yct+40)
    display_text(str(total), 24, xct+55, yct+40)
    display_text('Win Car1: ', 24, xct-75, yct+65, TEAL)
    display_text(str(car1), 24, xct+55, yct+65, TEAL)
    display_text('Win Car2: ', 24, xct-75, yct+90, GREEN)
    display_text(str(car2), 24, xct+55, yct+90, GREEN)
    display_text('Draw: ', 24, xct-75, yct+115)
    display_text(str(draw), 24, xct+55, yct+115)
    pygame.display.update()
    pygame.time.wait(2000)
